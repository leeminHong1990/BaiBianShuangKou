...
class iRoomOperation(object):
	...
	# c2s
	def createRoom(self, room_mode, game_round, insert_card, deal_mode, score_mode, changeSeat, best_phase, is_agent):
		DEBUG_MSG("create room, user_id:{0} room_mode:{1}, game_round:{2}, insert_card:{3}, deal_mode:{4}, score_mode:{5}, changeSeat:{6}, best_phase:{7}, is_agent:{8}".format(self.userId,  room_mode, game_round, insert_card, deal_mode, score_mode, changeSeat, best_phase, is_agent))
		...
		def callback(content):
			...
			try:
				...
				card_cost, diamond_cost = switch.calc_cost(game_round, room_mode)
	...
	def createRoomCallback(self, room, err = None):
		if room:
			...
			if room.is_agent == 1:
				...
				card_cost, diamond_cost = switch.calc_cost(room.game_round, room.room_mode)
				if switch.DEBUG_BASE:
					callback('{"card":99, "diamond":999}'.encode())
				else:
	...
	def quitRoomSucceed(self, player_info_list = []):
		DEBUG_MSG('@' * 30)
		DEBUG_MSG('avatar quit room succeed!')
		self.room = None
		if getattr(self, 'client', None):
			self.client.quitRoomSucceed(player_info_list)
	...
	def startGame(self, controllerIdx, keyCard, cards, swap_list):
		if getattr(self, 'client', None):
			self.client.startGame(controllerIdx, keyCard, cards, swap_list)

	def postOperation(self, idx, aid, real_card_list, instead_card_list):
		if getattr(self, 'client', None):
			self.client.postOperation(idx, aid, real_card_list, instead_card_list)
	...
	def notifyCooperationCards(self, cooperation_idx, cards):
		if getattr(self, 'client', None):
			self.client.notifyCooperationCards(cooperation_idx, cards)
	# c2s
