...
var ResultUI = UIBase.extend({
	...
	show_by_info:function(finalPlayerInfoList){
		...
		this.show(function(){
			...
			for (var i = 0; i < finalPlayerInfoList.length; i++) {
				scoreList.push(finalPlayerInfoList[i]["score"])
			}
			var maxScore = Math.max(scoreList)
			var maxIdxList = []
			for (var i = 0; i < finalPlayerInfoList.length; i++) {
				if (finalPlayerInfoList[i]["score"] >= maxScore && finalPlayerInfoList[i]["score"] > 0) {
					maxIdxList.push(i)
				}
			}
			for(var i = 0; i < 4; i++){
				var cur_player_info_panel = self.player_panels[i];
				if (finalPlayerInfoList.length <= i) {
				...
				self.update_player_info(finalPlayerInfo["idx"], finalPlayerInfo, i, maxIdxList);
			}
			// for(var i = 0; i < finalPlayerInfoList.length; i++){
	...
	update_player_info:function(serverSeatNum, finalPlayerInfo, idx, maxIdxList){
		var player = h1global.entityManager.player();
		...
		if (maxIdxList.indexOf(i) >= 0) {
			ccui.helper.seekWidgetByName(cur_player_info_panel, "winner_img").setVisible(true)
		}
	},
});
