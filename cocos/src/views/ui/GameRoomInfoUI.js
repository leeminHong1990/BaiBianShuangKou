...
var GameRoomInfoUI = UIBase.extend({
	...
	update_round:function(){
		...
		this.roominfo_panel.getChildByName("roomround_label").setString("局数:" + h1global.entityManager.player().curGameRoom.curRound.toString() + "/" + h1global.entityManager.player().curGameRoom.maxRound.toString())
	},
	...
	update_roommode:function(){
		...
		// cc.log("111111111111",roommode_panel);
		// roommode_bg_img.addTouchEventListener(function(sender, eventType){
		// 	if(eventType == ccui.Widget.TOUCH_ENDED){
		// 		roommode_panel.setVisible(!roommode_panel.isVisible());
		// 	}
		// });
		if (h1global.entityManager.player().curGameRoom.room_mode == 0) {
			this.roominfo_panel.getChildByName("roommode_label").setString("模式:普通双扣")
		}else{
			this.roominfo_panel.getChildByName("roommode_label").setString("模式:百变双扣")
		}
