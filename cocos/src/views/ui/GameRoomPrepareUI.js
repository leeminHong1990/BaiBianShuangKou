...
var GameRoomPrepareUI = UIBase.extend({
	...
	initUI:function(){
		...
		for(var i = 0; i < 4; i++){
			// var serverSeatNum = player.server2CurSitNum(i);
			cc.log( player.curGameRoom.playerInfoList)
		...
		var share_title = '房间号【' + player.curGameRoom.roomID.toString() + '】';
		var share_desc = '我在[双扣]开了房间，快来一起玩吧。';
		share_desc += '最高翻线' + String(player.curGameRoom.best_phase) +',' + const_val.INSERT_CARD[player.curGameRoom.insert_card] +'插牌,' + const_val.DEAL_MODE[player.curGameRoom.deal_mode] +'发牌,' + const_val.SCORE_MODE[player.curGameRoom.scoreMode] + ',还缺' + String(player.curGameRoom.getNeedPlayerNum()) + '人'
        ...
        prepare_btn.addTouchEventListener(function (sender, eventType) {
            if (eventType == ccui.Widget.TOUCH_ENDED){
                cc.log("player.serverSitNum " ,player.serverSeatNum);
	...
	update_player_info_panel:function(serverSeatNum, playerInfo){
		if(serverSeatNum < 0 || serverSeatNum > 3){
			return;
		}
		var player = h1global.entityManager.player();
	...
	update_player_state:function(serverSeatNum, state){
		...
		cc.log("serverSeatNum, state : ",serverSeatNum, state);
		var player = h1global.entityManager.player();
		var player_info_panel = this.rootUINode.getChildByName("player_info_panel" + player.server2CurSitNum(serverSeatNum).toString());
	...
	playEmotionAnim:function(serverSeatNum, eid){
		...
		if(serverSeatNum < 0){
			...
			player_info_panel = this.rootUINode.getChildByName("player_info_panel" + h1global.entityManager.player().server2CurSitNum(serverSeatNum));
		}
